...
const AiMenu: React.FC<AiMenuProps> = ({ isOpen, setIsOpen, activeListId, activeListName }) => {
  ...
  return (
    <aside className={`fixed top-0 right-0 h-full w-88 max-w-[92vw] bg-[#FDF6EC] border-l border-[#F2D9B3] flex flex-col justify-between p-5 shadow-2xl transition-transform duration-300 ease-in-out z-50 ${
      ...
      {/* FIXED: Changed to flex-1 and overflow-hidden to bound the structural layout calculations correctly */}
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <button
          ...
          disabled={loading}
          className="mb-6 flex w-full items-center justify-center gap-2 rounded-xl border border-[#F2D9B3] bg-white px-4 py-2.5 text-sm font-medium text-[#8A4B12] shadow-sm transition hover:bg-[#FFF3E6] disabled:cursor-not-allowed disabled:opacity-70 shrink-0"
        >
          <RefreshCw size={16} className={loading ? 'animate-spin' : ''} />
          {loading ? 'Generating...' : 'Generate Report'}
        </button>

        {/* FIXED: Swapped 'min-h-48' for 'flex-1' and 'overflow-y-auto' so it expands fully to the text footer and scrolls inside itself */}
        <div className="mb-4 flex flex-1 flex-col justify-start rounded-2xl border border-[#F2D9B3] bg-white p-4 text-left shadow-sm overflow-y-auto">
      ...
      <div className="mt-auto flex items-center gap-2 rounded-xl border border-[#F2D9B3] bg-white p-2.5 shadow-sm shrink-0">
